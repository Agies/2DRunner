...
#region SectionFlags
	...
	public static Pathfinding.Serialize.SerializeSettings serializationSettings = Pathfinding.Serialize.SerializeSettings.All;
...
#region Styles
	...
	public void OnEnable () {
		...
		if (!Application.isPlaying || script.astarData.graphs == null) {
			LoadGraphs ();
		}
	}
	...
	public void DrawSerializationSettings () {
		...
		if (GUILayout.Button ("Save to file")) {
			...
			if (path != "") {
				...
				Pathfinding.Serialize.AstarSerializer.SaveToFile (path,bytes);
			}
		}
		...
		if (GUILayout.Button ("Load from file")) {
			...
			if (path != "") {
				byte[] bytes;
				try {
					bytes = Pathfinding.Serialize.AstarSerializer.LoadFromFile (path);
		...
		//if (GUILayoutx.DrawID ("settings")) {
			...
			script.threadCount = EditorGUILayout.Toggle (new GUIContent ("Multithreading","Use multithreading or not.\n" +
				"Using multithreading will run the pathfinding in a separate thread which boosts performance on 2+ core computers (most computers today, even many smartphones).\n" +
			 	"SingleCoreOptimize #define is enabled so only the choice between running in the Unity thread or a separate thread is available."),script.threadCount == ThreadCount.One)
	...
	public void SaveGraphsAndUndo (EventType et = EventType.Used) {
		...
		if (!enableUndo || e == null || script.astarData.GetData() == null) {
			uint checksum;
		...
		if ((e.button == 0 && (et == EventType.MouseDown || et == EventType.MouseUp)) || (e.isKey && (e.keyCode == KeyCode.Tab || e.keyCode == KeyCode.Return)) || et == EventType.ExecuteCommand) {
			...
			byte[] bytes2 = SerializeGraphs ();
			
			if (bytes.Length != bytes2.Length) {
				Debug.Log ("Length diff.");
			} else {
	...
	public byte[] SerializeGraphs (out uint checksum) {
		Pathfinding.Serialize.SerializeSettings settings = Pathfinding.Serialize.SerializeSettings.Settings;
