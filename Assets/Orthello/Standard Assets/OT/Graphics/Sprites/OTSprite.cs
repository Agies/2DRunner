...
{		
    ...
    public Material material
    {
        get
        {
            ...
            if (Application.isPlaying)
                ...
                return otRenderer.sharedMaterial;
        }
        set
        {
			if (otRenderer == null)
            ...
            if (Application.isPlaying)
                otRenderer.material = value;
            else
                otRenderer.sharedMaterial = value;
        }
    }
    ...
    {		
        ...
        if (mr != null && (_tintColor_ != tintColor || _alpha_ != alpha))
            isDirty = true;
            ...
            else
            {
		        ...
		        if (_image_ != image) 
				{
					if (!baseSize.Equals(Vector2.zero))		
		            	...
		            	size = new Vector2(image.width * (1/OT.sizeFactor), image.height * (1/OT.sizeFactor)) * OT.view.sizeFactor;
					baseSize = new Vector2(image.width * (1/OT.sizeFactor), image.height * (1/OT.sizeFactor));
				}				
